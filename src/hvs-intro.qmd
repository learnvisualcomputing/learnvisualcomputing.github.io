...
The optical signal impinging on the retina is called the **optical image**, which is a 2D continuous signal in that at any position on the retinal surface we can ask: how much optical power is there here^[The power at an infinitesimal point is called *irradiance*; see @sec-chpt-mat-basics-radiometry.]?
...
For our purpose, "imperfections" introduced by eye optics (aberration and diffraction) can be modeled by the Point Spread Function (PSF) of the optical system, which we will see later in @sec-chpt-imaging-optics-modeling.
...
Photoreceptors absorb incident photons; once a photon is absorbed, it could generate electrical responses through the process of **phototransduction** cascade.
...
We will have a lot to say about this process later in @sec-chpt-hvs-receptor.
...
The response here is represented by the photocurrent, the change of current that flows into the photoreceptor as a result of photon absorption, which we will talk about in detail later in @sec-chpt-hvs-receptor.
...
The entire @sec-chpt-hvs-color is devoted to color vision; for now, let us just appreciate how different cones have different wavelength selectivities.
...
where $I_b$ is the background luminance and $I$ is the object luminance.
...
where $I_{max}$ and $I_{min}$ are the highest and lowest luminance, respectively, of the signal.
...
The CSF in @fig-csf characterizes a single RGC.
We can also measure the behavioral CSF through psychophysics.
The most famous such experiment was done by @campbell1968application.
@fig-csf_behavioral (left) shows the results.
We can see that the behavioral CSF also has a bandpass shape.
The chart in @fig-csf_behavioral (right) varies spatial frequency horizontally and contrast vertically.
By tracing the boundary between visible and invisible gratings^[Note that the boundary is *not* the CSF.], you can see that your sensitivity is generally highest at intermediate spatial frequencies.
Visibility also depends strongly on viewing distance: moving the display toward or away from your eyes changes the spatial frequency of the grating on your retina.

![Left: behavioral CSF, where squares correspond to square-wave gratings, and circles correspond to sine-wave gratings; empty and solid markers represent two different luminance levels that differ by four orders of magnitude; from @campbell1968application[Fig. 4]. Right: a chart that simultaneously varies frequency (horizontally) and contrast (vertically).](figs/csf_behavioral){#fig-csf_behavioral width="100%"}

The empty and solid markers represent two different luminance levels that differ by four orders of magnitude.
The sensitivity, however, varies by less than one order of magnitude.
This has to do with light adaptation, which we will discuss shortly.

The behavioral CSF is generally wider than that of individual RGCs (compare, for instance, @campbell1968application[Fig. 15] for cat RGC CSFs and @pasternak1981luminance[Fig. 2] for behavioral CSFs on cats).
@campbell1968application hypothesized that the behavioral CSF is determined by the collective contributions of different groups of RGCs, each of which has a different CSF that peaks at a particular frequency.
The existence of these spatial frequency channels has also been supported by later experiments on *contrast adaptation*, where the sensitivity toward a particular frequency (and its adjacent frequencies) reduces as one stars at gratings of that frequency long enough, showing that different frequency channels are modulated independently.

CSF allows us to study the joint effect of spatial frequency and contrast in detecting a patterned signal.
...
Minimally detectable contrast being constant over different background intensities is called the **Weber's law** [@fechner1860elemente]; in fact, this is why $\frac{\Delta I}{I_b}$ is called the Weber contrast.
A direct interpretation of the Weber's law is that a signal needs to be proportionally stronger at high ambient light levels for the signal to be barely detectable.
That is, our visual system is **desensitized** at higher ambient light levels.
...
All in all, however, Weber's law holds reasonably well under a very wide range of normal lighting conditions that we encounter in everyday life.
...
When @eq-weber is written in the log-log domain, as is plotted in @fig-rgc_adaptation, we have:
...
To account for this, the Weber's law is extended, phenomenologically, to take the following general form:
...
The contribution of Weber is the empirical observation that the just-noticeable signal strength is proportional to the existing signal strength (@eq-weber).
We now know that Weber's law holds because of light adaptation, where RGCs encode contrast information.
Fechner was Weber's student and gave a theoretical explanation of Weber's law that predates, and is now replaced by, the prevailing contrast theory.
Nevertheless, it is still widely used and is practical useful in many settings, so we will briefly discuss it here.
...
where $I$ is the current signal strength, and $p(I)$ is the internal response under $I$.
$C$ allows us to account for the fact that there is a minimal signal strength $I_0$ at which Weber's law holds^[$C = p(I_0) - \ln(I_0)$, so @eq-fechner1 can also be written as $p(I) = \ln(I/I_0) + p(I_0)$ or, equivalently, $p(I) - p(I_0) = \ln(I) - \ln(I_0)$, which has an intuitive interpretation: the perceived signal strength change is proportional to absolute signal strength change in the logarithmic domain.].
The reason Fechner came up with this relationship is because

$$
    \frac{\text{d}p(I)}{\text{d}I} = \frac{1}{I}.
$$ {#eq-fechner2}

Why is this relevant?
We need three more assumptions. First, humans require a minimal change in internal response, $\Delta p(I)$, for $I+\Delta I$ to be behaviorally detactable against $I$.
Second, $\Delta p(I)$ is a constant regardless of $I$.
Third, $\Delta p(I)$ and the corresponding $\Delta I$ are sufficiently small so that we can treat them as infinitesimal (equivalent to $\text{d}p(I)$ and $\text{d}I$).
Therefore, we have:

$$
    \frac{\Delta I}{\Delta p(I)} \approx \frac{\text{d}I}{\text{d}p(I)} = I.
$$ {#eq-fechner3}

Since $\Delta p(I)$ is a constant, we have $\Delta I \propto I$, which is essentially @eq-weber.

@eq-fechner2 is called the Fechner's law or Weber-Fechner law.
Stevens' power law generalizes Fechner's law by using a power law to describe the relationship between perceived signal strength and the physical signal strength [@stevens1957psychophysical; @stevens1961honor]:

$$
    p(I) = kI^a.
$$ {#eq-stevens}

The power-law relationship is simply more expressive.
By changing $a$, we can describe both sub-linear ($0<a<1$, like Fechner's law) and super-linear ($a>1$) relationships.
The power-law relationship is widely used in practice.
For instance, most color spaces and video communication standards use the power-law equation to encode luminance into digital values (@sec-chpt-hvs-cori-cube-step2).
