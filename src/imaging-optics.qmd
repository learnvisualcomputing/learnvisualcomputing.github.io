...
## Lenses {#sec-chpt-imaging-optics-lens}
...
With a little radiometry (which we will omit but refer to @bohren2006fundamentals[chap. 4.1.6] for the derivation), we can show that the radiance after refraction $L_r$ relates to the incident radiance $L_i$ by:
...
## Aberrations {#sec-chpt-imaging-optics-lens-abe}
...
It turns out that the equation still holds even if the lens is thick (i.e., the distance between the two surfaces is not negligible), even though the definition of the focal length would have to be slightly more complicated than @eq-lmeq_2 [@hecht2016optics, chap. 6.1].
...
## Linear System Modeling {#sec-chpt-imaging-optics-modeling}

How do we model the image formed on the sensor plane?
We could trace rays out of points in the scene, but it has many limitations.
...
Third, things like diffraction that go beyond geometrical optics need special treatment if not straight up impossible using ray tracing.
...
where $\mathcal{F}$ and $\mathcal{F}^{-1}$ denote Fourier transform and inverse Fourier transform.
