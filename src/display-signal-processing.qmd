...
The OOTF that maps the scene luminance to displayed luminance is called tone mapping.
...
Some of these transfer functions are determined by the design of the imaging and display hardware (&#9313; and &#9321;), others by image encoding and decoding format (e.g., &#9314;, &#9315; &#9317;, &#9318;), and the remainder by the image processing algorithm (&#9316;), which is the component over which we have the greatest (or sometimes the only) control and, thus, where the tone mapping operator is implemented.
...
In practice, another challenge in implementing a good TMO is that it has limited visibility of the end-to-end workflow: it receives information only from &#9325; rather than the scene directly and sends information only to &#9327; rather than directly to the display.
So any information loss before or after the TMO would hurt the tone mapping quality.
...
### Camera-Intrinsic Transfer Function
...
@eq-mono_model can be thought of as the intrinsic transfer of the imaging system (&#9313; in @fig-ootf).
...
$$
    P_{cam} = 
    \begin{bmatrix}
        T_{cam-R}(\Phi_s(\lambda)) \\
        T_{cam-G}(\Phi_s(\lambda)) \\
        T_{cam-B}(\Phi_s(\lambda))
    \end{bmatrix},
$$ {#eq-hoetf}

where $P_{cam}$ is the pixel color in the camera RAW space, $T_{cam-R}(\cdot)$, $T_{cam-G}(\cdot)$, and $T_{cam-B}(\cdot)$ represent the camera transfer functions for the red, green, and blue channel, respectively, and $\Phi_s(\lambda)$ represents the SPD of the incident light in the scene.
We have three transfer functions here because there are three different spectral sensitivity functions (@sec-chpt-imaging-sensor-color-goal).
The $T_{cam}$s are defined to account for quantization: if using a 10-bit encoding, each channel in $P_{RAW}$ is bounded between 0 and 1023.
...
Gamma encoding makes better use of the limited bit budget by reducing the perceptual quantization errors for the low luminance range.

The encoding function $f_{L \mapsto V}$ that maps a luminance-linear signal $L$ to a digital value $V$ that is actually saved in an image file is also called the Opto-Electro Transfer Function, or **OETF** (&#9314; in @fig-ootf).
Note, however, the information being manipulated technically live only in the electrical domain: both $L$ and $V$ are electrical signals (i.e., variables modified by a computer program).
...
In practice, the OETF is applied after a color space conversion (CSC) from the raw camera space to a standard color space such as sRGB or Display P3 (which we cover in @zhu2022cam), each of which specifies an OETF.
...
* $P_{sRGB}$ ($\mathbb{Z}^3 \in [0, 255]^3$) is the color in the sRGB space. $\text{diag}(255)$ implicitly performs a quantization step, which is omitted here for simplicity.

### Tone-Mapping Operator: the Controllable Part of OOTF

When an OETF-encoded image is later processed, we usually use OETF^-1^ to recover the original luminance (&#9315; in @fig-ootf).
...
The image processing pipeline can be thought of as a custom OOTF, as it processes information linear to luminance (&#9316; in @fig-ootf).
This is usually the part of the entire processing pipeline that we get to control, so it is where we implement our TMO to influence the overall OOTF.
Sometimes, the TMO is also called the artistic OOTF as it allows artistic expression of image processing [@iturbt2100-3].
Mathematically:
$$
\begin{aligned}
...
This is because usually the processing stages before the TMO do not keep the absolute luminance information because of all the normalizations.
@sec-disp-sp-tm discusses challenges facing implementing a good TMO and typical solutions.

@eq-eetf assumes that we are given a digital image to begin with.
If possible, we want to implement tone mapping at the end of a rendering pipeline or a camera signal processing pipeline before a digital image is saved (@sec-chpt-imaging-isp).
That is, the TMO can be implemented after step &#9313; and before OETF.
At this stage, we have direct access to analog or high-precision (e.g., 10 bits) luminance information before it is quantized to a lower (e.g., 8) bit depth in an image.

### EOTF

After the TMO, each image pixel is mapped to an intended (relative) luminance.
...
Over the years, there have been many EOTFs defined in various standards.
Rec. 1886 [@iturbt1886] defines an EOTF that is meant to give a good approximation of the hardware-intrinsic transfer function of CRT displays, and Rec. 2100 defines EOTFs that are meant for HDR workflows, where absolute luminance is tracked.
Rec. 601, Rec. 709, Rec. 2020, sRGB, and Display P3 define only the OETF but not the EOTF, since they are meant to encode images at the capturing time, rather than for display.
...
For example, if the input image is encoded in sRGB, the pixels remain in the sRGB space after applying the TMO. If the display operates in the Display P3 color space, a CSC must be performed from linear sRGB to linear Display P3, after which the P3 EOTF is applied to obtain the digital pixel values.
...
* $P_{P3}$ ($\mathbb{Z}^3 \in [0, 1023]^3$) is the color in the P3 space, assuming 10-bit encoding (assuming $\text{diag}(1024)$ is defined to also perform quantization.).

<!-- Even though the TMO in @eq-eetf operates completely within the sRGB space, by cascading @eq-eetf and @eq-eotf we can see that the TMO effectively maps pixels from the color space where the input image is encoded ($P_{sRGB}$ here) to the color space where the tone-mapped image is to be displayed ($P_{P3}$ here), so the TMO can also be thought of as:
$$ \mathcal{M}_{sRGB \rightarrow P3}: P_{sRGB\_linear} \mapsto P_{P3\_linear}, $$
-->

### Display-Intrinsic Transfer Function
...
The relationship between the digital value $D$ and the emitted power spectrum $\Phi_d(\lambda)$ or the corresponding luminance is what we call the display-intrinsic transfer function $T_{disp}$.
...
In practice, $T_{disp}$ is affeced by many factors (such as variation in manufacturing, the particular driving circuit design, etc.), and is usually measured offline rather than modeled analytically.

![The display-intrinsic transfer function of four inorganic LEDs (R, G, B, and W). From @miller2019color[Fig. 7.2].](figs/eotf){#fig-eotf width="50%"}
...
Given the display-intrinsic transfer function $T_{disp}$, converting from P3-encoded pixels to DAC inputs requires two steps.

* First, we use the EOTF (in this case part of the Display P3 standard) to decode the actual luminance intended to be displayed (&#9318; in @fig-ootf).
* Second, we perform a CSC from the Display P3 space to the display native space, after which we invert $T_{disp}$ to obtain the digital value to send to the DACs (&#9319; in @fig-ootf).
...
\begin{aligned}
    ...
    D &= 
    \begin{bmatrix}
        T_{disp-R}^{-1}(P_{disp}(R)) \\
        T_{disp-G}^{-1}(P_{disp}(G)) \\
        T_{disp-G}^{-1}(P_{disp}(B))
    \end{bmatrix}, \\
...
* $D$ is the DAC inputs, one for each channel since each sub-pixel might have a different hardware-intrinsic transfer function.
...
In this way, after going through $T_{disp}$ (&#9321; in @fig-ootf) the luminance emitted by the display matches the intended luminance.
...
Cascading all the equations from @eq-hoetf to @eq-heotf, we can see that the TMO we implement (@eq-eetf) eventually dictates the mapping from the scene SPD to the displayed SPD, and the overall OOTF (or end-to-end TMO) can be thought of as:

$$
    OOTF_{Overall}: \int\Phi_s(\lambda)V(\lambda)\text{d}\lambda \mapsto \int\Phi_d(\lambda)V(\lambda)\text{d}\lambda,
$$

where $V(\lambda)$ is the luminous efficiency function.

This mapping is achieved by cascading a sequence of transfer functions:
$$
    OOTF_{Overall} = T_{cam} \circ OETF \circ OETF^{-1} \circ OOTF_{TMO} \circ EOTF^{-1} \circ EOTF \circ T_{disp},
$$

where $T_{cam}$ and $T_{disp}$ are intrinsic transfer functions of the camera and display, respectively, and they perform the actual signal transduction between the electrical and optical domains, whereas OETF (on the imaging side) and the EOTF (on the display side) are both meant to manipulate electrical signals.
Depending on whethere an electrical signal carries luminance-linear information or (quantized/digital) luminance non-linear value, it is given the symbol "O" or "E".

Specifically, we use OETF to encode scene luminance (and potentially save to a file) and later use OETF^-1^ to recover the scene luminance (from the file).
We can then manipulate the luminance-linear information by, e.g., implementing an TMO.
The tone-mapped luminance information is then encoded in digital values using EOTF^-1^ (and potentially saved to another file).
The encoded values will later be processed by EOTF to recover the intended luminance information and sent to the display.
The "O"s in $OOTF_{Overall}$ refer to actual optical signals.
Sometimes people refer to $OETF^{-1} \circ OOTF_{TMO} \circ EOTF^{-1}$ as the Electro-Electro Transfer Function (**EETF**), since this composite operation takes a non-linear value as input and generates another non-linear value.

Importantly, OETF and EOTF need not be an inversion of each other.
That is, $OETF$ does not have to be $EOTF^{-1}$ and $EOTF$ does not have to be $OETF^{-1}$, either.
...
As seen above, however, TMOs are usually implemented without access to the absolute luminance information.
...
The recent ITU-R Rec. 2100 standard [@iturbt2100-3] defines EOTFs and OETFs using *absolute* luminance (and specifies that the peak display luminance).
This allows us to directly control the output luminance within the TMO, assuming we encode image using Rec. 2100 and the viewing display supports it.
...
Another way we can control the absolute luminance is through software that allow us to interactively adjust the TMO, such as the famous Curves tool in Photoshop and Lightroom.
...
ICC profiles use the notion of Tone Response Curve (**TRC**) to refer to the EOTF, which is invertible.
...
Similarly, we can assume that the display will use the TRC in its own profile to turn digital pixels to luminance-linear signals, which means we should use the inversion of the TRC to encode pixel values.
...
The gamut mapping algorithm is usually implemented in &#9317; in @fig-ootf after the CSC before applying the EOTF^-1^.
