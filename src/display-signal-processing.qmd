...
### Tone-Mapping Operator: the Controllable OOTF
...
The image processing pipeline can be abstracted as a custom OOTF, as it processes information linear to luminance (&#9316; in @fig-ootf).
This OOTF is usually the part of the entire processing pipeline that we get to control, so it is where we implement our TMO to influence the overall OOTF.
Sometimes, the TMO is also called the artistic OOTF [@iturbt2100-3].
Mathematically:
$$
\begin{aligned}
...
After the TMO OOTF, each image pixel is mapped to an intended (relative) luminance.
...
Rec. 601, Rec. 709, Rec. 2020, sRGB, and Display P3 technically define only the OETF but not the EOTF, since they are meant to be used on the image encoding side, rather than for display.
...
Cascading all the equations from @eq-hoetf to @eq-heotf, we can see that the TMO we implement (@eq-eetf) eventually dictates the mapping from the scene SPD to the displayed SPD, so the overall OOTF (or end-to-end TMO) can be thought of as:
...
Both the OETF on the imaging side and the EOTF on the display side are both meant to transform, back and forth, between continuous luminance-linear values and quantized/digital values non-linear to luminance.
We use OETF to encode scene luminance to a file and later use OETF^-1^ to recover the scene luminance from the file.
We can then manipulate the luminance-linear information to, e.g., implement an TMO.
The tone-mapped luminance information is then encoded in digital values using EOTF^-1^.
The encoded values will later be processed by EOTF to recover the intended luminance information.
So the general processing pipeline looks like:

$$
    OOTF_{Overall} = T_{cam} \circ OETF \circ OETF^{-1} \circ OOTF_{TMO} \circ EOTF^{-1} \circ EOTF \circ T_{disp},
$$

where $T_{cam}$ and $T_{disp}$ are intrinsic transfer functions of the camera and display, respectively, and they, again, perform the actual signal transduction between the electrical and optical domains, whereas all other transfer functions manipulate electrical signals.
The "O"s and the "E"s of the transfer functions on the right-hand side of the equation refer to electrical signals that are linearly and non-linearly proportional to luminance, respectively.
