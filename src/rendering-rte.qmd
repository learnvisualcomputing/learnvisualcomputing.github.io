...
Observe, in the second case, that $w_0 = \alpha_0$ and $w_{i+1} = w_i \frac{\alpha_{i+1}(1-\alpha_i)}{\alpha_i}$, so there is generally a unique assignment of the $\alpha$ values for a given weight combination.
...
Note, however, that if we impose the constraint that $\alpha\in[0, 1]$, we are effectively constraining the weights in the second case, too.
