...
The Y CMF is intentionally designed to match the CIE 1924 Luminous Efficiency Function (LEF), so that by looking at the Y value of a color, we can tell what its *relative* luminance is (refer to @sec-chpt-hvs-color-oppo-light for the definition of the LEF and its various caveats).
...
For the reference, we also show the three primary lights and the white point of the CIE 1931 RGB color space as well as the Planckian locus, which is the path of chromaticities that an ideal black-body radiation would take at different temperatures (@fig-blackbody_colors).
...
This is called gamut mapping, which we will discuss in @sec-disp-sp.
...
These are questions explored in the interactive tutorial [@zhu2022cube], which you are invited to go through.
...
The convention is to set the Y value of white to be 1 and scale everything else accordingly, but of course the actual luminance of white (and any other color) depends on the actual device used.
...
Therefore, a uniform allocation of the bits would mean uniformly quantizing the power range.
...
Therefore, uniformly quantizing the power range would lead to a *non-uniform* quantization of the brightness range, leading to large *perceptual* quantization error when encoding low luminances.
...
A common method is to first model the brightness level ($B$) as a power-law function (@eq-stevens) of the raw channel value ($v \in [0, 1]$) by $B=v^{1/2.2}$ and then quantize $B$ uniformly.
...
The sRGB standard [@anderson1996proposal; @srgbweb; @srgbiec] essentially uses this approach with one slight tweak to avoid numerical issues when $v$ is small.
...
where $v \in [0, 1]$ is one of the three RGB channels.
An 8-bit quantization is then applied to $B \in [0, 1]$ to bring each channel value to an integer between 0 and 255 assuming an 8-bit encoding.
...
% there is no way to directly measure brightness vs. luminance, so we can only measure TVI. see my google docs note on "Weber’s Law and Sensitivity in Psychophysics vs. Physiology".
...
In addition, the ambient light and the resulting flare from the display in the viewing environment also affect how a digital value $v$ actually translates to luminance $L$ entering our visual system.
...
That is why the sRGB standard specifies the peak luminance of the display (white point) as 80 cd/m$^2$ and that the mean illuminance level of the viewing environment to be 64 lux.
Presumably this means that at this particular luminance range (0 to 80 cd/m$^2$ plus whatever we get from flare), the relationship between $B$ and $L$ roughly follows the power law.
When actually viewing an sRGB image, both conditions are rarely met, so take all these with a huge grain of salt [@srgbweb].

ITU-R Rec. 2100 addresses the first issue by defining OETFs that are based on absolute luminance  [@miller2013perceptual; @iturbt2100-3] and are, thus, more perceptually relevant (especially at high luminance levels), but still, it needs to say things like "avoid light falling on the screen" in the standard to constrain the ambient lighting in the viewing environment.
We discuss Rec. 2100 in more detail in @sec-disp-sp-proc.

Second, light adaptation [@sec-chpt-hvs-adaptations-light] also plays a role as the sensitivity of our visual system depends on the luminance levels that we are adapted to.
Therefore, both sRGB and Rec. 2100 have specified the *recommended* luminance level of the immediate surround of the display (and in the case of Rec. 2100, the luminance level of the rest of the viewing environment), which affects the adaptation state of the observer.
Again, these are rarely met in practice.
...
A practical interpretation of a dicrimination contour is that with an ellipse all the colors are undiscriminable with respect to the center, reference color.
